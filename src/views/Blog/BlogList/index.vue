  ...
  <div class="blog-list-container app-container">
    ...
    <div class="firstRow">
      <div class="leftPart">
        ...
        <el-input v-model="keyword" style="flex: 1;" placeholder="请输入关键词" />
        <el-button icon="el-icon-search" type="primary" plain @click="handleFilteBlog" />
      </div>
      <div class="rightPart">
        <el-button type="danger" plain @click="handleDeletChecked">删除选中</el-button>
      </div>
    ...
    <!-- 数据表格 -->
    <!-- 数据表格 -->
    <el-table ref="multipleTable" v-loading="loading" :data="data" border style="width: 100%" @selection-change="handleSelectionChange">
      <!-- 序号 -->
      <el-table-column
        label="序号"
        width="50"
      ...
      <el-table-column label="文章名称" width="160" align="center">
        <template slot-scope="scope">
          <el-popover placement="top-start" trigger="hover">
            ...
            />
            <a slot="reference" href="#" target="_blank" @click.prevent="handleGoToBlog(scope.row)">{{ scope.row.title
            }}</a>
          </el-popover>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        if (this.categories.length === 0) {
        ...
        } else if (this.keyword === '' && (this.chooseCategories.length === this.categories.length || this.chooseCategories.length === 0)) {
          // 没有进行筛选
          ...
          // 仅关键词筛选
          const resp = await getBlogByPager(this.currentPage, this.eachPage, -1, this.keyword)
          this.data = resp.data.rows
          this.count = resp.data.total
        } else {
          // 有分类，进行筛选
          const result = await Promise.all(this.chooseCategories.map(id => getBlogByPager(this.currentPage, this.eachPage, id || -1, this.keyword)))
          this.data = []
          this.count = 0
          result.map(item => {
